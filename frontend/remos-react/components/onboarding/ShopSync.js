...
export default function ShopSync() {
  ...
  const handleSyncOrders = async () => {
    ...
    setError(null);
    try {
      await callApi('/shop/orders/sync/bol', 'POST');
  ...
  const handleSyncOffers = async () => {
    ...
    setError(null);
    try {
      await callApi('/shop/offers/export/csv', 'GET');
  ...
  const订单同步已启动 = syncStatus.orders.startsWith('Order sync initiated');
  const报价同步已启动 = syncStatus.offers.startsWith('Offer export and sync initiated');
  // A more robust check might involve specific success flags from the backend if available
  const canCompleteStep = 订单同步已启动 && 报价同步已启动;
