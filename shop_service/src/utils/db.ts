...
export const runMigrations = async (): Promise<void> => {
  ...
  try {
    client = await currentPool.connect();
    console.log('Connected to DB for running shop service migrations.');
    ...
    `);
    console.log('Checked/created shop_schema_migrations table.');
    ...
    for (const file of migrationFiles) {
      ...
      const res = await client.query('SELECT version FROM shop_schema_migrations WHERE version = $1', [version]);
      if (res.rowCount === 0) {
        ...
        try {
          ...
          await client.query('INSERT INTO shop_schema_migrations (version) VALUES ($1)', [version]);
    ...
    console.log('All shop service migrations processed.');
  } catch (error) {
    console.error('Shop service migration process failed:', error);
    throw error; // Re-throw to indicate failure
  } finally {
    if (client) {
      client.release();
      console.log('Shop service migration DB client released.');
    }
  }
};
